...
def write_protocol(protocol_path, wells, volumes): 
    ...
    try: 
        ...
        print("ERROR: Could not write to protocol file")


# MAIN TEMPLATE CONTENTS ----------------------------------------------------------  

# ask user to input source csv files (between 1 and 5 files allowed)
source_csvs = []
source_wells = {}
source_volumes = {} 
csv_input_count = 1   
...
# loop to gather source csvs
print("\nEnter csv file paths and deck locations for source plates below (1-5 plates allowed)")
print("-----------------------------------------------------")
while True: 
    current_path = input(f"CSV FILE PATH {csv_input_count} (type 'done' to continue): ").strip()
    if current_path == 'done': 
        ...
        while True: 
            current_deck_loc = input("DECK LOCATION (integer 2-6 inclusive): ") 
            try: 
                int_deck_loc = int(current_deck_loc.strip())

                if int_deck_loc < 2 or int_deck_loc > 6: 
                    print("Error: deck location must be an integer between 2 and 6 inclusive")
                    continue
                if int_deck_loc in used_deck_loc: 
                    print(f"Error: deck location {int_deck_loc} already used")
                    continue
                break

            except: 
                print("Error: deck location must be an integer")
                continue
    
    current_tuple = (int_deck_loc, current_path)
    used_deck_loc.append(int_deck_loc)
    source_csvs.append(current_tuple)

    csv_input_count += 1
    if csv_input_count == 6:
...
# print deck layout -----
print("\nDECK LAYOUT: ")
print("\t1: DESTINATION PLATE")
print("\tSOURCE PLATES: ")
for loc, path in source_csvs: 
    print(f"\t\t{loc}: {path}")
print("\tTIP BOXES: ")
for i in range(num_boxes): 
    print(f"\t\t{tip_box_loc[i]}: 20uL tip box")
# ------------------------

if input("\nGenerate protocol now? (y/n): ").strip() == 'y':
    # user input, where to write protocol
    generate_folder = input("Path to folder that will store the generated file: ").strip()
    protocol_filename = input("OT-2 protocol filename (exclude .py): ").strip() + str(".py")
    # create output file path 
    try: 
        file_to_create = os.path.join(generate_folder, protocol_filename)
        write_protocol(file_to_create, source_wells, source_volumes)
    except: 
        print("Error: Coud not resolve output protocol file path")
...
#This data structure is generated by the given csv. 
#It informs the OT-2 about the source wells, destination wells, and the volumes.

# exp_dict = { 
#     "source_wells": df["Source"],
#     "volumes": df["Volume"]
#             }

#print(list(zip(exp_dict["source_wells"], exp_dict["volumes"])))
...
# extra code snippets (DELETE LATER)


        # # load the source plates  (find a cleaner way to do this)
        # if len(source_locs) == 1: 
        #     source1 = protocol.load_labware(SOURCE_PLATE_TYPE, source_locs[0])
        # elif len(source_locs) == 2: 
        #     source1 = protocol.load_labware(SOURCE_PLATE_TYPE, source_locs[0])
        #     source2 = protocol.load_labware(SOURCE_PLATE_TYPE, source_locs[1])
        # elif len(source_locs) == 3: 
        #     source1 = protocol.load_labware(SOURCE_PLATE_TYPE, source_locs[0])
        #     source2 = protocol.load_labware(SOURCE_PLATE_TYPE, source_locs[1])
        #     source3 = protocol.load_labware(SOURCE_PLATE_TYPE, source_locs[2])
        # elif len(source_locs) == 4: 
        #     source1 = protocol.load_labware(SOURCE_PLATE_TYPE, source_locs[0])
        #     source2 = protocol.load_labware(SOURCE_PLATE_TYPE, source_locs[1])
        #     source3 = protocol.load_labware(SOURCE_PLATE_TYPE, source_locs[2])
        #     source4 = protocol.load_labware(SOURCE_PLATE_TYPE, source_locs[3])
        ...
        #     source4 = protocol.load_labware(SOURCE_PLATE_TYPE, source_locs[3])
        #     source5 = protocol.load_labware(SOURCE_PLATE_TYPE, source_locs[4])
        # elif len(source_locs) < 1: 
        #     print("Error: no source locations to initialize")
        #     raise
        # else: 
        #     print("Error initializing source plates")
        #     raise



         # #load the source plate. 
        # source_plate = protocol.load_labware(SOURCE_PLATE_TYPE, SOURCE_PLATE_SLOT)
        
        #load the source wells. 
        #source_wells = [source_plate.wells_by_name()[well] for well in source_wells]
        
        #start the transfer, home the robot when it's done.
        #pipette.transfer(list(volumes), source_wells, destination_well, blow_out=False, new_tip="always")